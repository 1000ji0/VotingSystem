...
if st.session_state.stage == "home":
    ...
    #### ⚠️ **주의사항** ⚠️
     🔺 이 서비스는 다양한 투표 방식을 경험하기 위한 목적으로 제작되었습니다.
     🔺 원활한 결과 비교를 위해 **후보자와 투표자 모두 2명 이상**으로 설정해주세요. (콩도르세 등 일부 방식은 후보 2명 이상 필요)
     🔺 투표 결과는 방식에 따라 동률이 나올 수 있습니다. 동률 발생 시, 제공되는 타이브레이킹 옵션을 사용하거나 다른 투표 방식의 결과를 참고할 수 있습니다.

    ## 4가지 투표 방법을 소개할게요

    #### 🏆 보르다 방식
    **"순위 숫자의 합이 가장 작은 후보가 승리!"**  
    각 투표자가 후보들에게 매긴 순위값(1위=1점, 2위=2점...)을 모두 더합니다. 이 합계가 가장 **작은** 후보가 선택됩니다.
    - 장점: 모든 순위가 결과에 반영됩니다.
    - 단점: 전통적인 보르다(높은 순위에 높은 점수)와는 반대 방식입니다.

    #### ⚖️ 내쉬 방식
    **"모든 투표자의 선호도 점수를 곱해서 최대화!"**  
    각 후보에 대해 모든 투표자가 부여한 선호도 점수(0~10점)를 곱합니다. 이 곱한 값이 가장 **큰** 후보가 선택됩니다. (0점은 계산 시 0.00001로 처리)
    - 장점: 만장일치에 가까운 강한 선호를 가진 후보에게 유리할 수 있습니다.
    - 단점: 한 명이라도 0점에 가까운 점수를 주면 전체 곱이 매우 작아질 수 있습니다.

    #### 🥊 콩도르세 방식
    **"1순위 대결에서 가장 많이 이긴 후보는?"**  
    모든 후보들을 두 명씩 짝지어 가상 대결을 합니다. 각 대결에서는 투표자들이 해당 후보들 중 누구를 **1순위**로 선택했는지만 비교합니다. 이렇게 pairwise 대결에서 가장 많은 승리를 한 후보가 선택됩니다.
    - 장점: 다수결 원칙을 pairwise에 적용합니다.
    - 단점: 전체 순위를 고려하지 않고 1순위만 보며, 콩도르세 패러독스(순환) 발생 시 해석이 Colab 코드의 승자 결정 방식에 따릅니다.

    #### 💯 벤담 방식
    **"모든 선호도 점수의 총합이 가장 큰 후보!"**  
    각 투표자가 각 후보에게 부여한 선호도 점수(0~10점)를 모두 합산합니다. 이 총합이 가장 **큰** 후보가 선택됩니다.
    - 장점: 선호도의 강도를 직접적으로 반영합니다.
